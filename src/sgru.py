...
"""
Switching GRU for Interpretability based on
...
class HkSGRU(hk.RNNCore):
  """A switching GRU RNN core implemented using Haiku."""
  ...
  def __init__(
    ...
    self.gru_cores = [hk.GRU(hidden_size=self._hidden_size, name=f'switch_{i}') 
  ...
  def __call__(self, observations: jnp.ndarray, prev_state: jnp.ndarray):
    """
    Performs one step of the switching GRU computation.
    There are 4 possible values for observations at a timestep. 
    Each should have its own parameter set.
    """
    switch = (observations[..., 0] + 2 * observations[..., 1]).astype(int)

    all_outputs = []
    all_nextstates = []
    for i in range(self._num_switches):
        gru_output, next_state = self.gru_cores[i](jnp.zeros_like(observations), prev_state)
        all_outputs.append(gru_output)
        all_nextstates.append(next_state)

    stack_outs = jnp.stack(all_outputs, axis=0)
    stack_nexts = jnp.stack(all_nextstates, axis=0)

    batch_size = jnp.shape(prev_state)[0]
    br = jnp.arange(batch_size)
    y_hat = self.output_layer(stack_outs[switch, br])

    return y_hat, stack_nexts[switch, br]

  def initial_state(self, batch_size: Optional[int]):
    """
    Returns the initial hidden state for the GRU.
    """
    return self.gru_cores[0].initial_state(batch_size)


def train_sgru_network(
